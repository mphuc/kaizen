...
class ModelAccountCustomer extends Model {
	...
	public function update_R_Wallet_add($amount, $customer_id, $wallet){
			...
			INSERT INTO " . DB_PREFIX . "customer_r_wallet_payment SET
				...
				date_added = NOW(),
				date_end = DATE_ADD( NOW(), INTERVAL + 180 DAY)
			");
	...
	public function createPD($amount, $max_profit,$type_pd,$amount_usd){
		$this -> db -> query("
	...
	public function getTotalHistory_dayli($customer_id){
		$query = $this -> db -> query("
			SELECT count(*) AS number 
			FROM ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".intval($customer_id)."' AND wallet LIKE 'Dayli profit'
		");
	...
	public function getTotalHistory_binary($customer_id){
		$query = $this -> db -> query("
			SELECT count(*) AS number 
			FROM ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".intval($customer_id)."' AND wallet = 'Pairing profit'
		");
	...
	public function getTransctionHistory_binary($id_customer, $limit, $offset){
		$query = $this -> db -> query("
			SELECT *
			FROM  ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".$this -> db -> escape($id_customer)."' AND wallet = 'Pairing Profit'
			ORDER BY date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function getTransctionHistory_dayli($id_customer, $limit, $offset){
		$query = $this -> db -> query("
			SELECT *
			FROM  ".DB_PREFIX."customer_transaction_history
			WHERE customer_id = '".$this -> db -> escape($id_customer)."' AND wallet = 'Dayli profit' 
			ORDER BY date_added DESC
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function getPD_active($iod_customer){
		...
		$query = $this -> db -> query("
			SELECT max(amount_usd) as filled
			FROM ". DB_PREFIX . "customer_provide_donation
	...
	public function checkN_Wallet($id_customer){
		$query = $this -> db -> query("
			SELECT COUNT(*) AS number
			FROM  ".DB_PREFIX."customer_n_wallet
			WHERE customer_id = '".$this -> db -> escape($id_customer)."'
		");
		return $query -> row;
	}
	...
	public function insert_N_Wallet($id_customer){
		$query = $this -> db -> query("
			INSERT INTO " . DB_PREFIX . "customer_n_wallet SET
			customer_id = '".$this -> db -> escape($id_customer)."',
			amount = '0',
			date = DATE_ADD(NOW(),INTERVAL - 90 DAY)
		");
	...
	public function update_count_day_payment($id){
		
		$query = $this -> db -> query("
			UPDATE 	" . DB_PREFIX . "customer_r_wallet_payment SET count_day  = count_day + 1 WHERE id = '".$id."'
		");
	...
	public function update_n_Wallet_add_sub($amount , $customer_id, $add = false){
		if ($add) {
			$query = $this -> db -> query("	UPDATE " . DB_PREFIX . "customer_n_wallet SET
			amount = amount + ".intval($amount).",
			date = NOW()
			WHERE customer_id = '".$customer_id."'
		");
	...
	public function update_total_pd_node($amount , $customer_id, $date = false){
		if (!$date) {
			$query = $this -> db -> query("	UPDATE " . DB_PREFIX . "customer SET
			total_pd_node = total_pd_node - ".doubleval($amount)."
			WHERE customer_id = '".$customer_id."'
		");
	...
	public function getCustomer_lai3() {
		$query = $this -> db -> query("SELECT c.total_pd_node,c.customer_id FROM " . DB_PREFIX . "customer c  WHERE c.total_pd_node >0 ");
		return $query -> rows;
	}
	...
	public function count_getmaxPD_all(){
		$query = $this -> db -> query("
			SELECT count(*) number
			FROM  ".DB_PREFIX."customer_provide_donation
	...
	public function saveWithdrawpayment($customer_id,$amount,$addres_wallet,$amount_usd){
		$query = $this -> db -> query("
	...
	public function getmaxPD_customer($customer_id){
		$query = $this -> db -> query("
			SELECT max(amount_usd) AS number
			FROM  ".DB_PREFIX."customer_provide_donation
	...
	public function savetai_pd($amount,$max_profit,$type_pd,$amount_usd){
		$this -> db -> query("
			INSERT INTO ". DB_PREFIX . "customer_provide_donation_tai SET 
			customer_id = '".$this -> session -> data['customer_id']."',
			date_added = NOW(),
			filled = '".$amount."',
			date_finish =NOW(),
		...
		$pd_number = hexdec( crc32($pd_id) );
		$query = $this -> db -> query("
			UPDATE " . DB_PREFIX . "customer_provide_donation_tai SET 
				max_profit = '".$max_profit."',
				pd_number = '".$pd_number."'
				WHERE id = '".$pd_id."'
			");
