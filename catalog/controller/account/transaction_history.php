...
class ControllerAccountTransactionHistory extends Controller {
	...
	public function dayli() {
		...
		$ts_history = $this -> model_account_customer -> getTotalHistory_dayli($this -> session -> data['customer_id']);
		...
		$pagination -> num_links = 5;
		$pagination -> text = 'text';
		$pagination -> url = HTTPS_SERVER . 'dayli-profit.html&page={page}';
		$data['histotys'] = $this -> model_account_customer -> getTransctionHistory_dayli($this -> session -> data['customer_id'], $limit, $start);

		$data['pagination'] = $pagination -> render();


        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/account/transaction_history_dayli.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/account/transaction_history_dayli.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/account/transaction_history_dayli.tpl', $data));
        }
	}
}