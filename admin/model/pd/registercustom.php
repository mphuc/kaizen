...
class ModelPdRegistercustom extends Model {
	...
	public function get_all_invesment($limit, $offset){
		...
		$query = $this -> db -> query("
			SELECT A.*,B.username,B.wallet,C.transaction_hash
			FROM  ".DB_PREFIX."customer_provide_donation A INNER JOIN ".DB_PREFIX."customer B ON A.customer_id = B.customer_id LEFT JOIN ".DB_PREFIX."customer_invoice_pd C ON A.id = C.transfer_id WHERE A.status = 1
	...
	public function update_status_payment($customer_id,$status){

		$query = $this -> db -> query("
			UPDATE ".DB_PREFIX."customer_r_wallet_payment SET status = '".$status."' WHERE customer_id = '".$customer_id."'
		");
	}
	...
	public function get_all_paringbonus($limit, $offset){
		...
		$query = $this -> db -> query("
			SELECT A.*,B.username
			FROM  ".DB_PREFIX."customer_transaction_history A INNER JOIN ".DB_PREFIX."customer B ON A.customer_id = B.customer_id WHERE A.wallet = 'Refferal Commistion' 
			LIMIT ".$limit."
			OFFSET ".$offset."
		");
	...
	public function get_all_dailyprofix_all(){
		...
		$query = $this->db->query("
			SELECT  SUM(rpm.amount)/10000 as amount, rpm.addres_wallet AS addres_wallet, rpm.customer_id 
			FROM sm_customer_r_wallet_payment AS rpm
			WHERE count_day <= 60 AND status = 0
		");
		return $query->rows;
	}

	public function get_all_withdraw_all(){

		$query = $this->db->query("
			SELECT  SUM(rpm.amount)/100000000 as amount, rpm.addres_wallet AS addres_wallet, rpm.customer_id,id 
	...
	public function get_all_dailyprofix_all_customer(){

		$query = $this->db->query("
			SELECT  *
			FROM sm_customer_r_wallet_payment AS rpm
			WHERE status = 0 AND count_day <= 60
		");
		return $query->rows;
	}

	public function get_all_dailyprofix_all_customerss(){

		$query = $this->db->query("
			SELECT  addres_wallet as address, amount as amount
			FROM sm_customer_r_wallet_payment AS rpm
			WHERE status = 0 AND count_day <= 60
		");
		return $query->rows;
	...
	public function get_count_paringbonus(){
		...
		$query = $this -> db -> query("
			SELECT count(*) as number
			FROM  ".DB_PREFIX."customer_transaction_history WHERE wallet = 'Refferal Commistion'
		");
		return $query -> row;
	}
	...
	public function update_amount_r_payment($customer_id){
		
		$query = $this -> db -> query("
			UPDATE 	" . DB_PREFIX . "customer_r_wallet_payment SET amount  = 0 WHERE customer_id = '".$customer_id."'
		");
		
	}

	public function delete_withdraw($customer_id){
		
		$query = $this -> db -> query("
			DELETE FROM " . DB_PREFIX . "customer_withdraw_payment
		");
	...
	public function count_all_cn_show(){
		$query = $this -> db -> query("
			SELECT count(*) as number FROM " . DB_PREFIX . "customer_transaction_history WHERE wallet = 'Withdraw Matching Commistion' 
		");
		return $query -> row;
	}
	public function get_all_cn_show($limit,$offset){
		$query = $this -> db -> query("
			SELECT A.*,B.username FROM " . DB_PREFIX . "customer_transaction_history A INNER JOIN " . DB_PREFIX . "customer B ON A.customer_id = B.customer_id WHERE A.wallet = 'Withdraw Matching Commistion'  LIMIT ".$limit."
			OFFSET ".$offset."
		");
		return $query -> rows;
	}
	...
	public function update_count_day($id){
		$query = $this->db->query("
			UPDATE sm_customer_r_wallet_payment SET count_day = count_day + 1
			WHERE id = ".$id."
		");
		return $query;
	}
