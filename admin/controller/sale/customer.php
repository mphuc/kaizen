	...
	*/
	...
	//
		if (($this->request->server['REQUEST_METHOD'] == 'POST') ) {
			...
			$get_customer_id = $this -> model_sale_customer -> get_customer_id($this->request->post['username'],$this->request->get['customer_id']);
			if (count($get_customer_id) > 0)
			{
				$this->response->redirect($this->url->link('sale/customer/edit', 'token=' . $this->session->data['token']."&customer_id=".$this->request->get['customer_id']."#username" . $url, 'SSL'));
			}
	...
	if (isset($this->request->post['account_bank'])) {
		...
		if (isset($this->request->post['coin_token'])) {
			$data['wallet'] = $this->request->post['coin_token'];
		} elseif (!empty($customer_info)) {
			$data['coin_token'] = $customer_info['coin_token'];
		} else {
			$data['coin_token'] = '';
		}
